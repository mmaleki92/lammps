...
FixIndentWCA::FixIndentWCA(LAMMPS *lmp, int narg, char **arg) :
...
void FixIndentWCA::setup(int vflag)
{
...
void FixIndentWCA::min_setup(int vflag)
{
  post_force(vflag);
}
...
void FixIndentWCA::post_force(int /*vflag*/)
{
  ...
  // spherical indenter
  double epsilon = 1.0;  // Depth of the potential well (example value)
  double sigma = 1.0;   // Particle diameter (example value)
  double cutoff = pow(2.0, 1.0 / 6.0) * sigma;  // Default cutoff distance
  if (istyle == SPHERE) {
    // spherical indenter
    ...
    for (int i = 0; i < nlocal; i++) {
      if (mask[i] & groupbit) {
        ...
        // Apply cutoff: Only calculate force if r is within the cutoff
        if (r > cutoff) continue;

        // Inline WCA potential force calculation with cutoff consideration
        double r_inv = sigma / r;
        double r_inv6 = r_inv * r_inv * r_inv * r_inv * r_inv * r_inv;
        double r_inv12 = r_inv6 * r_inv6;
        double fmag = 24 * k * (2 * r_inv12 - r_inv6) / r;  // Use `k` as the strength of the WCA potential
        ...
        // Adjust indenter values (replace with WCA energy calculation if needed)
        indenter[0] -= k3 * dr * dr * dr; // Adjust this part as needed for WCA
        indenter[1] -= fx;
        indenter[2] -= fy;
        indenter[3] -= fz;
      }
...
void FixIndentWCA::post_force_respa(int vflag, int ilevel, int /*iloop*/)
{
...
void FixIndentWCA::min_post_force(int vflag)
{
  post_force(vflag);
}
...
double FixIndentWCA::compute_scalar()
{
...
double FixIndentWCA::compute_vector(int n)
{
  // only sum across procs one time
...
int FixIndentWCA::geometry(int narg, char **arg)
{
...
void FixIndentWCA::options(int narg, char **arg)
{
  scaleflag = 1;
  side = OUTSIDE;
...
bool FixIndentWCA::PointInsideCone(int dir, double *center, double lo, double hi, double rlo,
                                double rhi, double *x)
{
...
void FixIndentWCA::DistanceExteriorPoint(int dir, double *center, double lo, double hi, double rlo,
...
void FixIndentWCA::DistanceInteriorPoint(int dir, double *center, double lo, double hi, double rlo,
                                      double rhi, double &x, double &y, double &z)
{
...
void FixIndentWCA::point_on_line_segment(double *a, double *b, double *c, double *d)
{
  double ba[3], ca[3];
...
double FixIndentWCA::closest(double *x, double *near, double *nearest, double dsq)
{
